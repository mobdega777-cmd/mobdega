...
-- Drop the text version if it was created
DROP FUNCTION IF EXISTS public.apply_stock_deduction_for_order(text);

-- Replace the uuid version with the fix
...
BEGIN
  ...
  LOOP
    ...
    IF v_product.is_composite THEN
      ...
      -- Recalculate composite product stock (bottleneck of components)
      SELECT MIN(
        CASE 
