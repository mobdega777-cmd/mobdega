...
const CommerceStorefront = ({ commerceId, onBack }: CommerceStorefrontProps) => {
  ...
  // Photos gallery
  const [photos, setPhotos] = useState<Array<{ id: string; image_url: string; caption: string | null }>>([]);
          ...
          <div className="overflow-y-auto max-h-[60vh] pr-2 space-y-3">
            {photos.map((photo) => (
              <div key={photo.id} className="rounded-lg overflow-hidden">
                ...
                />
                {photo.caption && (
                  <p className="text-sm text-muted-foreground p-2 bg-muted/50">{photo.caption}</p>
                )}
              </div>
            ))}
          </div>
          <DialogFooter>
            <Button onClick={() => setShowPhotosModal(false)} className="w-full">
              Fechar
            </Button>
          </DialogFooter>
        </DialogContent>
